...
void GeometoryPolyline::CalcVertex()
{
    ...
    DirectX::XMStoreFloat3(&m_pVertices[0].pos, DirectX::XMVectorAdd(vCenter, DirectX::XMVectorScale(vCross, m_points[0].width * -0.5f)));
    DirectX::XMStoreFloat3(&m_pVertices[1].pos, DirectX::XMVectorAdd(vCenter, DirectX::XMVectorScale(vCross, m_points[0].width *  0.5f)));

    //----- 中間のポインタの頂点
    DirectX::XMVECTOR vProv = vCenter;
    vCenter = vNext;
    for (int i = 1; i < m_points.size() - 1; i++) {
        vNext = DirectX::XMLoadFloat3(&m_points[i + 1].pos);
        vVec = DirectX::XMVectorSubtract(vNext, vProv);
        vNormal = DirectX::XMLoadFloat3(&m_points[i].normal);
        // 念のため、外積の計算前に正規化する
        ...
        vCross = DirectX::XMVector3Normalize(vCross);
        // 外積の方向に向かって頂点を移動させる
        DirectX::XMStoreFloat3(&m_pVertices[i * 2 + 0].pos, DirectX::XMVectorAdd(vCenter, DirectX::XMVectorScale(vCross, m_points[i].width * -0.5f)));
        DirectX::XMStoreFloat3(&m_pVertices[i * 2 + 1].pos, DirectX::XMVectorAdd(vCenter, DirectX::XMVectorScale(vCross, m_points[i].width *  0.5f)));
        // 次のループに向けて値を更新
        vProv = vCenter;
        vCenter = vNext;
    }
    ...
    //----- 一番最後の地点の頂点
    vVec = DirectX::XMVectorSubtract(vCenter, vProv);
    vNormal = DirectX::XMLoadFloat3(&m_points.back().normal);
    vVec = DirectX::XMVector3Normalize(vVec);
    vNormal = DirectX::XMVector3Normalize(vNormal);
    vCross = DirectX::XMVector3Cross(vVec, vNormal);
    vCross = DirectX::XMVector3Normalize(vCross);
    DirectX::XMStoreFloat3(&m_pVertices[m_points.size() * 2 - 2].pos, DirectX::XMVectorAdd(vCenter, DirectX::XMVectorScale(vCross, m_points.back().width * -0.5f)));
    DirectX::XMStoreFloat3(&m_pVertices[m_points.size() * 2 - 1].pos, DirectX::XMVectorAdd(vCenter, DirectX::XMVectorScale(vCross, m_points.back().width *  0.5f)));
}
